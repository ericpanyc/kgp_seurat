...
merged_seurat <- merge(x = VS4, y = c(VS5,VS6,VS8), add.cell.id = samples)
...
meta_data$group <- ifelse(meta_data$orig.ident=="VS4","cystic",
                          ifelse(meta_data$orig.ident=="VS6","cystic","naive"))
...
# saveRDS(seurat_integrated,"data/4_6_VS_5_8_merge.RDS")

########################################################
cystic_naive <- readRDS("data/4_6_VS_5_8_merge.RDS")
...
clustree(cystic_naive, prefix = "integrated_snn_res.")

meta_data <- cystic_naive@meta.data 
Idents(cystic_naive) <- "integrated_snn_res.0.4"
...
cystic_naive@meta.data <- meta_data
cn_sc <- as.SingleCellExperiment(cystic_naive)
colnames(cn_sc) <- colData(cn_sc)$cells

# Get all cell id in a cluster
cluster_barcode <- lapply(levels(cystic_naive@meta.data$integrated_snn_res.0.4), function(i) cystic_naive@meta.data[which(cystic_naive@meta.data$integrated_snn_res.0.4 == i),"cells"])
...
names(pred.cell_fine_results) <- c(0:(length(cluster_barcode)-1))
# saveRDS(pred.cell_fine_results,"results/4&6_VS_5&8.RDS")
# pred.cell_fine_results <- pred.cluster_fine_results
# cluster level annotation fine labels
pred.cluster_fine <- SingleR(test = cn_sc, ref = hpca.se, labels = hpca.se$label.fine, method = "cluster", clusters = cystic_naive@meta.data$integrated_snn_res.0.4)
...
# write.xlsx(cluster_anno,"results/4&6_VS_5&8_singleR_cluster_level_anno.xlsx")
